...
const translateText = async (text: string, targetLanguage: string) => {
  try {
    const response = await fetch('https://api-free.deepl.com/v2/translate', { // Используем правильный URL для бесплатного API
      method: 'POST',
      headers: {
...
function CatFacts() {
  ...
  return (
    <CatFactsContainer>
      <ButtonCat onClick={()=>navigate("/projects")}> <SlActionUndo/> {" "} {t("projects.cat.buttonBack")}</ButtonCat>
        <ButtonCat onClick={getCatFact}>{t("projects.cat.buttonCat")}</ButtonCat>
      {isLoading ? <Spinner /> : 
      <ResultContainer isShowBlock={!!catFacts.length}>
        {catFactsElement}
      </ResultContainer>}
      <ButtonCat onClick={deleteCatFacts} disabled={!catFacts.length}>{t("projects.cat.buttonDeleteAll")}</ButtonCat>
