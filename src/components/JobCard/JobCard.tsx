  ...
  return (
    <JobCardWrapper>
     <ButtonControl position>
      <IconBox src={Skip} onClick={handlePrev} disabled={index === 0} scaleX/>
      <GoDotFill color="rgba(252, 241, 228, 0.7)" fontSize={"0.4vw"}/>
      <GoDotFill color="rgba(252, 241, 228, 0.7)" fontSize={"0.4vw"}/>
      <GoDotFill color="rgba(252, 241, 228, 0.7)" fontSize={"0.4vw"}/>
      <IconBox src={Skip} onClick={handleNext} disabled={index === jobs.length-1} />
      </ButtonControl>
     <CardComponent>
      <TitleH2Card>
        {jobs[index].title} &nbsp;
        <StyledA href={jobs[index].link} target="_blank"> {jobs[index].company} 
        </StyledA> &nbsp; {jobs[index].companyDescription}, &nbsp;{jobs[index].location}
      </TitleH2Card>
